...
class ManifestManagerView(View):
    ...
    def post(self, request):
        ...
        def create_psd_images(layer_construct, viewport,  depth=0, base_directory="/tmp/blowup"):
            def save_construct(layer_construct):
                logger.info("starting save_construct " + str(time.perf_counter_ns()))
                bytestream = io.BytesIO()
                layer_construct.composite().save(fp=bytestream, format="PNG")
                logger.info("saved to bytestream " + str(time.perf_counter_ns()))
                bytestream.seek(0)
                content_file = ContentFile(bytestream.read(), "{}.{}".format(slugify(layer_construct.name), "png"))
                logger.info("created contentfile " + str(time.perf_counter_ns()))
                return content_file

            files = []
            width = layer_construct.width
            height = layer_construct.height

            if(type(layer_construct) == Group):
                if(height >= viewport[3] and width >= viewport[2]):
                    files.append(save_construct(layer_construct))
                for layer in layer_construct:
                    files += create_psd_images(layer, viewport, depth + 1)
            else:
                if(height >= viewport[3] and width >= viewport[2]):
                    files.append(save_construct(layer_construct))
        ...
        def validate_image_type_and_create_canvas(file, extension):
            if extension in acceptable_types:
                try:
                    image_json, image_id, file_url = create_image(file)
                except Exception as e:
                    return None

                return create_canvas(image_json, file_url, os.path.splitext(file.name)[0], image_id)
            else:
        ...
        if operation == "create":
            ...
            for f in files:
                ...
                if extension == ".psd":
                    logger.info("starting opening psd " + str(time.perf_counter_ns()))
                    with PSDImage.open(f) as psd:
                        logger.info("opened psd " + str(time.perf_counter_ns()))
                        for layer_construct in psd:
                            images = create_psd_images(layer_construct, (psd.viewbox[0] * 0.02, psd.viewbox[1], int(psd.viewbox[2]* 0.3), int(psd.viewbox[3]* 0.3)))
                            for image in images:
                                canvas = validate_image_type_and_create_canvas(image, "." + image.name.split(".")[-1].lower())
                                canvases += [canvas] if canvas is not None else []
                else:
                    canvas = validate_image_type_and_create_canvas(image, os.path.splitext(file.name)[0])
                    canvases += [canvas] if canvas is not None else []
